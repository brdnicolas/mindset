import { Avatar, NavbarLabel } from '@/components'
import { useNavigate } from 'react-router-dom'
import { isActivePath, MAIN_ROUTES, SECONDARY_ROUTES } from '@/utils/routing/routing'
import { Icon } from '@iconify/react'
import { Command } from '@/components/atoms/command/Command'
import clsx from 'clsx'
import { useAppContext } from '@/contexts/app/app.provider'
import { setIsNavigationTiny } from '@/contexts/app/app.actions'

export const DesktopNavbar = () => {
  const { isNavigationTiny, dispatch: dispatchApp } = useAppContext()
  const navigate = useNavigate()

  const goTo = (path: string) => {
    navigate(path)
  }

  const handleOnAvatarClick = () => {
    navigate('/settings')
  }

  const handleToggleNavigationSize = (isTiny: boolean) => {
    dispatchApp(setIsNavigationTiny({ isNavigationTiny: isTiny }))
  }

  return (
    <div
      className={clsx(
        'h-full relative bg-gray-700 rounded-r-4 p-6 flex flex-col',
        isNavigationTiny ? 'w-fit' : 'w-fit'
      )}
    >
      <div className="flex items-center justify-between w-full">
        <Avatar
          onClick={handleOnAvatarClick}
          className="cursor-pointer"
          src="data:image/png;base64,"
          alt="student"
        />
        {isNavigationTiny ? null : (
          <Icon
            onClick={() => handleToggleNavigationSize(true)}
            icon="solar:square-double-alt-arrow-left-linear"
            className="text-6 text-gray-500 cursor-pointer"
          />
        )}
      </div>
      {isNavigationTiny ? (
        <div className="mt-10 h-6">
          <div className="flex flex-col gap-1 items-center cursor-pointer">
            <Icon icon="solar:magnifer-outline" className="text-6 text-gray-500" />
          </div>
        </div>
      ) : (
        <div className="flex items-center mt-10 w-full gap-2 cursor-pointer">
          <Icon icon="solar:magnifer-outline" className="text-6 text-gray-500" />
          <p className="text-gray-500 whitespace-nowrap">Recherche rapide</p>
          <div className="flex gap-1 ml-auto">
            <Command>CMD</Command>
            <Command>K</Command>
          </div>
        </div>
      )}
      {isNavigationTiny ? (
        <div className="absolute top-7 py-2 rounded-r-3 -right-3 bg-gray-700">
          <Icon
            onClick={() => handleToggleNavigationSize(false)}
            icon="solar:double-alt-arrow-right-outline"
            className="text-6 text-gray-500 cursor-pointer mx-auto"
          />
        </div>
      ) : null}
      <div className="mt-10 flex flex-col gap-2">
        {MAIN_ROUTES.map((route) => (
          <NavbarLabel
            key={route.name}
            iconName={route.iconName}
            onClick={() => goTo(route.path)}
            isActive={isActivePath(route.path)}
            className={isNavigationTiny ? 'justify-center' : ''}
          >
            {isNavigationTiny ? null : route.name}
          </NavbarLabel>
        ))}
      </div>
      <div className="w-full flex flex-col gap-2 mt-auto">
        <hr className="h-[1px] w-full border-gray-500" />
        {SECONDARY_ROUTES.map((route) => (
          <NavbarLabel
            key={route.name}
            iconName={route.iconName}
            onClick={() => goTo(route.path)}
            isActive={isActivePath(route.path)}
            className={isNavigationTiny ? 'justify-center' : ''}
          >
            {isNavigationTiny ? null : route.name}
          </NavbarLabel>
        ))}
      </div>
    </div>
  )
}
